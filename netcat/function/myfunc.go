...
func SendMessage(message, nameClient string, sender net.Conn) {
	...
	for client, name := range Clients {
		clientFormat = fmt.Sprintf("[%s][%s]:", time.Now().Format(time.DateTime), name)
		if client != sender {
			client.Write([]byte("\n"))
			Message2clents := fmt.Sprintf("[%s][%s]:%s", time.Now().Format(time.DateTime), nameClient, (message))
...
func JoinOrLeft(name, msg string, sender net.Conn) {
	...
	for cl, myName := range Clients {
		clientFormat = fmt.Sprintf("[%s][%s]:", time.Now().Format(time.DateTime), myName)
		if cl != sender {
